...
public class Mapper : IIncrementalGenerator
{
	...
	internal record MapperInfo
	{
		public MapperInfo(TypeDeclarationSyntax syntax, INamedTypeSymbol symbol, SemanticModel semanticModel)
		{
			...
			this.Namespace = Factory.FindNamespace(syntax) ?? "MissingNamespace";
			this.SafeHintName = Factory.SafeHintName(semanticModel, $"{this.Namespace}.{this.Type.Name}").ResultName;

			List<string> usingStatements = new();
			Factory.UsingStatements(usingStatements, syntax, semanticModel, this.Namespace, []);
	...
	private static void Execute(SourceProductionContext context, MapperInfo inType)
	{
		...
		try
		{
			...
			{Factory.WithStringBuilder(inType.UsingStatements)}
