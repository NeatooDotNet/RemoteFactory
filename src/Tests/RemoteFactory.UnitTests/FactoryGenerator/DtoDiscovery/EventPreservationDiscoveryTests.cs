...
";
    ...
    [Fact]
    public void RegistrarOutput_OrdinallySorted_RegardlessOfDeclarationOrder()
    {
        // The registrar is one file aggregating many events — deterministic (ordinal)
        // emission order keeps generated bytes stable across builds
        // (ContinuousIntegrationBuild; plan review B4).
        var source = @"
using Neatoo.RemoteFactory;

namespace TestNamespace
{
    public record ZebraEvent(int Id) : FactoryEventBase;
    public record AlphaEvent(int Id) : FactoryEventBase;
}
";
        var tree = EventRegistrarTree(Run(source));

        var alphaIndex = tree.IndexOf("PreserveType<global::TestNamespace.AlphaEvent>()", StringComparison.Ordinal);
        var zebraIndex = tree.IndexOf("PreserveType<global::TestNamespace.ZebraEvent>()", StringComparison.Ordinal);

        Assert.True(alphaIndex >= 0 && zebraIndex >= 0, "both events must be emitted");
        Assert.True(alphaIndex < zebraIndex, "emissions must be ordinally sorted, not declaration-ordered");
    }

    [Fact]
    public void SameNamedBaseInOtherNamespace_NotMatched()
    {
        // Base matching is by fully-qualified name — a decoy FactoryEventBase in a
        // consumer namespace must not trigger the event pipeline (plan review B2).
        var source = @"
namespace MyApp
{
    public abstract record FactoryEventBase;

    public record FakeEvent(int Id) : FactoryEventBase;
}
";
        var all = AllTrees(Run(source));

        Assert.DoesNotContain("FakeEvent", all);
        Assert.DoesNotContain("NeatooEventPreservationRegistrar", all);
    }
