...
public class LoggingVerificationTests
{
    ...
    public async Task Verify_RemoteCreate_FullRoundTrip()
    {
        ...
        try
        {
            var factory = client.ServiceProvider.GetRequiredService<IRemoteCreateTarget_SimpleFactory>();
    ...
    public async Task Verify_RemoteFetch_WithParameters()
    {
        ...
        try
        {
            var factory = client.ServiceProvider.GetRequiredService<IRemoteFetchTarget_SimpleFactory>();
    ...
    public async Task Verify_LocalFactory_NoRemoteCall()
    {
        ...
        try
        {
            var factory = local.ServiceProvider.GetRequiredService<IRemoteCreateTarget_SimpleFactory>();

            loggerProvider.Clear();
            var result = await factory.Create(99);
    ...
    public async Task Verify_WriteOperation_InsertSerialization()
    {
        ...
        try
        {
            var factory = client.ServiceProvider.GetRequiredService<IRemoteSaveTarget_SimpleFactory>();

            // Create a new object that needs insert
            var obj = new RemoteSaveTarget_Simple { IsNew = true };

            loggerProvider.Clear();

            // Save it - this should serialize the object to the server
            var saved = await factory.Save(obj);
    ...
    public async Task Verify_ServiceInjection_InFactoryMethod()
    {
        ...
        try
        {
            var factory = client.ServiceProvider.GetRequiredService<IRemoteCreateTarget_WithServiceFactory>();

            loggerProvider.Clear();
            // This method has [Service] IService parameter
            var result = await factory.Create(77);
            ...
            report.AppendLine($"ServiceWasInjected: {result?.ServiceWasInjected}");
    ...
    public async Task Verify_AllEventIds_CapturedDuringOperations()
    {
        ...
        try
        {
            ...
            var createFactory = client.ServiceProvider.GetRequiredService<IRemoteCreateTarget_SimpleFactory>();
            var fetchFactory = client.ServiceProvider.GetRequiredService<IRemoteFetchTarget_SimpleFactory>();
            var saveFactory = client.ServiceProvider.GetRequiredService<IRemoteSaveTarget_SimpleFactory>();

            // Run various operations
            await createFactory.Create(1);
            await fetchFactory.Fetch(2);

            var obj = new RemoteSaveTarget_Simple { IsNew = true };
            await saveFactory.Save(obj);
    ...
    public async Task GenerateComprehensiveReport()
    {
        ...
        try
        {
            ...
            var createFactory = client.ServiceProvider.GetRequiredService<IRemoteCreateTarget_SimpleFactory>();
            var createWithServiceFactory = client.ServiceProvider.GetRequiredService<IRemoteCreateTarget_WithServiceFactory>();
            var fetchFactory = client.ServiceProvider.GetRequiredService<IRemoteFetchTarget_SimpleFactory>();
            var saveFactory = client.ServiceProvider.GetRequiredService<IRemoteSaveTarget_SimpleFactory>();
            ...
            report.AppendLine("1. Remote Create (with param)");
            await createFactory.Create(1);

            report.AppendLine("2. Remote Create (with param and service)");
            await createWithServiceFactory.Create(2);

            report.AppendLine("3. Remote Fetch (with param)");
            await fetchFactory.Fetch(3);

            report.AppendLine("4. Write - Insert new object");
            var obj = new RemoteSaveTarget_Simple { IsNew = true };
            await saveFactory.Save(obj);
