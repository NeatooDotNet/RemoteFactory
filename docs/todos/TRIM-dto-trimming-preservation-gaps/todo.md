...
6. `[Remote]` method bodies and their server-only dependencies are absent from a publish-trimmed client for **every** factory shape — including `[Execute]` static factories and `[FactoryEventHandler<T>]` classes, where they are retained today — proven in the trimmed harness, not inferred. [TRIM-008]
...
| 008 | Draft | [Registrar-DAM over-preservation fix](./plans/008-registrar-dam-over-preservation.md) | TRIM-005 plan review: `[Execute]` and `[FactoryEventHandler<T>]` registrar attributes name the consumer's class, so DAM retains every method incl. `[Remote]` bodies — release-blocking, falsifies AC4's docs. Folded into the arc 2026-08-12 (reverses the 2026-08-11 plan-mode routing) |

Execution order: 004 → 001 → 002 → 003 → 007 → 005 → 006 → 008 (rows listed in execution order; numbering stays monotonic by creation). Branching: todo/plan docs commit on the `TRIM` branch; each plan's implementation gets its own branch off `TRIM`. (TRIM-003's red verification and TRIM-007's fix merged together via PR #71.)
...
| 2 | **Release held (AC4 + AC5)** — version stays `1.6.1`, no v1.7.0 notes | Reopens when item 1 merges | zTreatment PCB-003 blocked since July. Deliberate trade: consumer unblock-time vs. publishing false IP guidance |
...
| 6 | **B9 — harness cannot verify the relay-handler leg** (no relay-handler target touches a server-only service) | Tied to item 1 | Item 1 would ship fixed but unverified |
| 7 | **Falsified TRIM-005 story in live artifacts** — `.github/workflows/build.yml:111-112`, `TrimmingTests/README.md:31`, `TrimTestCommands.cs:35`. The CI grep's `(?<!I)ServerOnlyRepository` exemption is justified by a diagnosis this arc disproved | Fold into item 1; the exemption must be re-derived then | Pointing at an Abandoned plan whose own Abandonment Reason says the story is wrong — the exact drift TRIM-007's code review caught three times |
| 8 | **Falsified-doc inventory is incomplete** — also `docs/client-server-architecture.md:133` and `skills/RemoteFactory/references/trimming.md:14`. Conversely `skills/.../class-factory.md:318,333-334` and `advanced-patterns.md:227` were **over-listed** as falsified; they describe class factories, which trim correctly | Fold into item 1's doc pass | Item 1 will miss them unless added. The skill must be corrected separately — repo `CLAUDE.md` requires it to be self-contained |
...
| 18 | **`async` generated `Local*` factory methods retain their server-only bodies under trimming** — measured 2026-08-13 on the new Save/Can\* harness leg. `[Remote] Insert/Update/Delete` bodies, their `[Service]` port interface, and their string literals all survive on a trimmed client. Sync `Local*` methods in the same assembly do not. Roots are the registrar's own **unguarded** `AddScoped<{X}Delegate>` closures, so this is not the registrar-DAM defect and the TRIM-008 holder fix does not touch it | **Needs a decision.** Distinct mechanism from TRIM-008, and it falsifies the *class-factory* leg — the shape the docs present as safe. Blocks AC6 as currently worded | The flagship IP-protection claim is false for every aggregate root with async write operations, which is most of them. Larger consumer-facing blast radius than TRIM-008 itself |
