...
|---|---|---|---|
| 1 | **Registrar-DAM over-preservation** — `[Remote]` bodies for `[Execute]` and `[FactoryEventHandler<T>]` classes ship to the browser decompilable | Built-in plan mode (user decision 2026-08-11); **release-blocking**, so it cannot be silently dropped | Highest-cost item. Its only record is a Discovery Log entry that archives when this todo closes — give it a durable home if the todo closes before the fix lands |
