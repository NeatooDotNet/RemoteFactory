...
- Deserialization path unchanged: parameterized-ctor types are claimed by `RecordBypassConverterFactory` (existing detection rule matches the bucket rule); parameterless DTOs keep flowing through `NeatooJsonTypeInfoResolver.CreateObject`.
- Roslyn incremental-generator discipline: model types stay value-equatable (`EquatableArray`) so pipeline caching doesn't regress.
...
1. Replace the walker's parameterless-ctor gate with bucket classification — parameterless → Register bucket, parameterized-public-ctor → PreserveType bucket — shared across return types, non-service parameters, and nested property descent (descending into both buckets' types). Retire the dead `WalkEventRoot` in favor of this shared walk and fix the stale file-header comment that still names the removed relay-handler caller.
2. Thread the ctor-shape distinction through the discovery pipeline: method-level discovery → per-type aggregation/dedupe → model builder → all three factory models, preserving value equality for incremental caching.
