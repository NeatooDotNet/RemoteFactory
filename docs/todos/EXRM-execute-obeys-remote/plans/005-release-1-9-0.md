...
- [ ] The repo's `CLAUDE.md` release process names the real version property, says release content is sourced from the arc's plans rather than a commit scan, and states a nav_order rule that can actually be followed — mirrored in `index.md`'s template `[explicit-skip: doc prose]` · Should
...
Line-ending note for the reviewer: a blanket CRLF normalisation was applied and then reverted on `src/Directory.Build.props`, which git stores LF; its diff is now exactly the three intended property changes (7 insertions / 4 deletions). The eleven release-note pages and `CLAUDE.md`/`index.md` normalised cleanly and show only their content edits.
