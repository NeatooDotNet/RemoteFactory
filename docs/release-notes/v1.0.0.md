...
**None from v0.23.1.** All 1.0 additions are new APIs. The `RelayedEvents` property on `RemoteResponseDto` is nullable and defaults to `null`, so existing wire payloads and older clients are unaffected.

## Migration Guide

Upgrading from v0.23.1 requires no code changes.
