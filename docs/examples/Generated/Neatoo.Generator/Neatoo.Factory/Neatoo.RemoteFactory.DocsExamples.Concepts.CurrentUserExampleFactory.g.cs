...
namespace Neatoo.RemoteFactory.DocsExamples.Concepts
{
    ...
    internal class CurrentUserExampleFactory : FactorySaveBase<ICurrentUserExample>, IFactorySave<CurrentUserExample>, ICurrentUserExampleFactory
    {
        ...
        public virtual Task<ICurrentUserExample?> LocalSave2(ICurrentUserExample target, CancellationToken cancellationToken = default)
        {
            if (target.IsDeleted)
            {
                if (target.IsNew)
                {
                    return Task.FromResult(default(ICurrentUserExample));
                }
